...
export const union = <T>(srcIterable: Iterable<T>) => (
    targetIterable: Iterable<T>
): Iterable<T> => uniq<T>()(concat(srcIterable)(targetIterable));

export const unionBy = <T, P>(
    transFn: (item: T) => P,
    srcIterable: Iterable<T>
) => (targetIterable: Iterable<T>): Iterable<T> =>
...
export const intersectionBy = <T, P>(
    transFn: (item: T) => P,
    srcIterable: Iterable<T>
) =>
    transformIterable<T, T, Set<P>>({
...
export const xor = <T>(srcItearble: Iterable<T>) => (
    targetIterable: Iterable<T>
) => {
...
export const xorBy = <T, P>(
    ...
    return transformIterable<T, T, Map<P, T>>({
        ...
        getInitialState: () =>
            toES6Map((item: T) => [transFn(item), item])(srcIterable)
    })(uniq<T>()(targetIterable));
};

export const concat = <T>(srcIterable: Iterable<T>) => (
    targetIterable: Iterable<T>
): Iterable<T> => {
    return {
        *[Symbol.iterator]() {
...
export const xprod = <P>(srcIterable: Iterable<P>) => <T>(
    targetIterable: Iterable<T>
): Iterable<[T, P]> => {
    return {
        *[Symbol.iterator]() {
...
export const zip = <P>(srcItearble: Iterable<P>) => <T>(
    targetIterable: Iterable<T>
): Iterable<[T, P]> => {
    return zipWith(
        (item1: T, item2: P) => [item1, item2] as [T, P],
        srcItearble
    )(targetIterable);
};

export const zipWith = <T, P, R>(
    zipFn: (targetItem: T, srcItem: P) => R,
    srcIterable: Iterable<P>
